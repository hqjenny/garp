...
abstract class RoCC()(implicit p: Parameters) extends Module
{
...
def inject(db: AdvTester[_], target: HellaCacheResp): Unit = {
    db.wire_poke(target.data, data)
    db.wire_poke(target.tag, tag)
    db.wire_poke(target.cmd, cmd)
  }
}
...
abstract class SimMemory(word_width: Int, depth: Int) extends Processable with MemoryOpConstants{
