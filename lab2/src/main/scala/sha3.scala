...
class Sha3Accel()(implicit p: Parameters) extends SimpleRoCC()(p) {
  ...
  //RoCC Interface defined in test_infrastructure.scala
  //cmd
  //resp
  //mem
  //busy
...
class Sha3AccelTests(c: Sha3Accel) extends AdvTester(c) {
  ...
  //run test
  until( Cmd_IHandler.isIdle && !dutBusy, 250 ) { instantMemory.process() }
...
/* TODO: still one bug
//Long bmark
    instantMemory.store_data(0, Array.fill(1024){BigInt("3736353433323130",16)})
    start_cycles = cycles
    instantMemory.reqs_handled = 0 
  ...
  peek(c.ctrl.read);
      ...
      instantMemory.process() }
    println("Test 12 took "+(cycles-start_cycles)+" cycles and had "+instantMemory.reqs_handled+" requests for a util of "+instantMemory.reqs_handled/(cycles-start_cycles).toDouble)
    var t12_resp = instantMemory.load_addr(8192)
    ...
    t12_resp = instantMemory.load_addr(8200)
    ...
    t12_resp = instantMemory.load_addr(8208)
    ...
    t12_resp = instantMemory.load_addr(8216)
