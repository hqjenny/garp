...
class CtrlModule(val W: Int, val S: Int)(implicit p: Parameters) extends Module() with MemoryOpConstants{
  ...
  if(buffer_sram){
    //TODO: LAB3
    //What address to pass into the SRAM?
    //Connect the output to io.buffer_out
  }else{
  ...
  is(m_wait){
    ...
    when(io.dmem_resp_val) {
      ...
      if(buffer_sram){
        //TODO: LAB3 - Handle return from dmem
      }else{
  ...
  is(m_pad) {
    ...
    //we have received all responses so the buffer is as full as it will get
      ...
      when(buffer_count < mindex && (pindex >= buffer_count) ){
        ...
        if(buffer_sram){
          //TODO: LAB3 - Increment pindex every other cycle
  ...
  if(fast_mem){
    when(io.dmem_resp_val) {
      when(io.dmem_resp_tag(4,0) < UInt(round_size_words)){
        //This is read response
        if(buffer_sram){
          //TODO: LAB3 - Handle Returns from dmem
        }else{
