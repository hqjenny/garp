...
class ControlBlockModule extends Module {
  ...
  io.read_demand_access := demand_access && ~((primes(3)(0)).toBool) 
  ...
  io.write_demand_access := demand_access && ((primes(3)(0)).toBool) && demand_access_type(1).toBool  

  //val prefetch_demand_access = Bool(false)
  io.prefetch_demand_access := demand_access && ((primes(3)(0)).toBool) && ~(demand_access_type(1).toBool)
  ...
  transfer_access := mem_mode && (primes(0)(0)).toBool && (primes(2)(0)).toBool 

  // val load_transfer_access = Bool(false)
  io.load_transfer_access := transfer_access && ~((primes(3)(0)).toBool) 

  // val store_transfer_access = Bool(false)
  io.store_transfer_access := transfer_access && ((primes(3)(0)).toBool) 
