...
#[stevo]: add keepout...
set_keepout_margin -outer {0.272 0.272 0.272 0.272} -all_macros
...
if {$ADD_FILLER_CELL } {
   ...
   if {$FILLER_CELL_METAL != ""} {insert_stdcell_filler -cell_with_metal $FILLER_CELL_METAL -connect_to_power $MW_POWER_NET -connect_to_ground $MW_GROUND_NET -respect_keepout}
   if {$FILLER_CELL != ""} {insert_stdcell_filler -cell_without_metal $FILLER_CELL -connect_to_power $MW_POWER_NET -connect_to_ground $MW_GROUND_NET -respect_keepout}
...
set_fix_hold_options \
  -effort high \
  -prioritize_min_delay \
  -preferred_buffer

focal_opt -hold_endpoints all -effort high -prioritize
...
if {[info exists env(DISPLAY)]} { 
 ...
 # Hide the "View Settings" toolbar for more screen real estate
 # gui_hide_palette  -parent $LW -type view_settings_toolbar

 gui_show_window -window ${MyLayout} -show_state {maximized}
 gui_show_window -window [gui_get_current_window -types Layout -mru] -show_state {maximized}

 gui_zoom -window [gui_get_current_window -types Layout -mru] -full 

 #DUCKY these don't seem available in our version yet
 #report_congestion -grc_based -by_layer -routing_stage global
 #gui_show_map -map "Global Route Congestion" -show true

 # Broken for now
 # gui_show_map -map "AREAPARTITION" -show true  
 report_congestion -grc_based -by_layer -routing_stage global
 gui_show_map -window [gui_get_current_window -types Layout -mru] -map {AREAPARTITION} -show {true}
 gui_write_window_image -format png -file $REPORTS_DIR/$ICC_CHIP_FINISH_CEL.congestion
 gui_write_window_image -window ${MyLayout} -format png -file $REPORTS_DIR/$ICC_CHIP_FINISH_CEL.congestion.window
 gui_show_map -window [gui_get_current_window -types Layout -mru] -map {AREAPARTITION} -show {false}

 gui_set_setting -window [gui_get_current_window -types Layout -mru] -setting vmName -value "Highlight"
 gui_set_setting -window [gui_get_current_window -types Layout -mru] -setting vmName -value {Hierarchy}
 gui_load_hierarchy_vm -level 1
 gui_write_window_image -format png -file $REPORTS_DIR/$ICC_CHIP_FINISH_CEL.hierarchy
 gui_write_window_image -window ${MyLayout} -format png -file $REPORTS_DIR/$ICC_CHIP_FINISH_CEL.hierarchy.window
 gui_load_hierarchy_vm -clear

 gui_show_map -map "cellDensityMap" -show true
 gui_set_map_option -map "cellDensityMap" -option grid_dimension -value 1
 gui_load_cell_density_mm
 gui_write_window_image -format png -file $REPORTS_DIR/$ICC_CHIP_FINISH_CEL.density
 gui_write_window_image -window ${MyLayout} -format png -file $REPORTS_DIR/$ICC_CHIP_FINISH_CEL.density.window
 gui_show_map -map "cellDensityMap" -show false 
