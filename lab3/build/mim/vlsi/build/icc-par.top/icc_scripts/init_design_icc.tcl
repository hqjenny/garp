...
########################################################################################
 ...
 read_verilog -top $DESIGN_NAME $ICC_IN_VERILOG_NETLIST_FILE
  ...
  # [ben] Copied from MIM flow reference
  change_macro_view -reference $HIERARCHICAL_DESIGNS -view FRAM
  set_top_implementation_options -block_references $HIERARCHICAL_DESIGNS 
  # RIMAS:trying to get rid of TIM-250 warnings
  #set case_analysis_propagate_through_icg true
  link -force
  echo "INFO: Done linking design"

  # [ben] These commands were not in the MIM reference
  #uniquify_fp_mw_cel 
...
if {$ICC_FLOORPLAN_INPUT == "USER_FILE"} {
   ...
   # [stevo]: set unit tile name if different from default "unit", then create floorplan to load in tile info
